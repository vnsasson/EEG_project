...
function [] = preprocessing_16_func(baseFileName,fullFileName,filepath_before,filepath_after,cap385,cap16)
    ...
    EEG=pop_chanedit(EEG, 'lookup',cap385,'load',{cap16,'filetype','autodetect'});
    EEG = eeg_checkset( EEG );
    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG); % save modifications 
...
%     pause
    ...
    EEG = pop_saveset( EEG, 'filename',baseFileName,'filepath',filepath_before);
    EEG = eeg_checkset( EEG );
    ...
    EEG = pop_saveset( EEG, 'filename',baseFileName,'filepath',filepath_after);
    EEG = eeg_checkset( EEG );
